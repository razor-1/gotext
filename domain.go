...
//GetAll processes all translations in the domain
func (do *Domain) GetAll() map[string]*store.Translation {
	lcData, _ := localizer.GetLocaleData(do.tag)

	do.trMutex.RLock()
	defer do.trMutex.RUnlock()

	all := make(map[string]*store.Translation)
